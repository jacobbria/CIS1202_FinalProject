...
int main()
{
	...
	do {
		...
		case 2:
			...
			break;
		case 3:
			searchFighter(allSubmissions, allKnockouts);
			break;
		case 4:
...
void searchFighter(const std::vector<Submission>& allSub, const std::vector<Knockout>& allKo) {
	...
	for (const auto& sub : allSub) {	// search the sub vector
		if (sub.getWinnerFirst() == firstName && sub.getWinnerLast() == lastName) {
			submissionCount++;
		}
	}

	for (const auto& ko : allKo) {	// search the KO vector
		if (ko.getWinnerFirst() == firstName && ko.getWinnerLast() == lastName) {
			knockoutCount++;
		}
	}

	// Display the results
	std::cout << firstName << " " << lastName << " has " << submissionCount << " submissions and " << knockoutCount << " knockouts." << std::endl;
}
